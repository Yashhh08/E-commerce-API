...
//    ADDRESS SERVICE
    ...
    @PostMapping("/address")
    public ResponseEntity<Address> addAddress(@RequestHeader("Authorization") String auth, @Valid @RequestBody Address address) throws CustomException{

        String token = auth.substring(7);

        String username = jwtUtil.extractUsername(token);

        Address added = addressService.addAddress(username, address);
    ...
    public ResponseEntity<String> deleteAddress(@RequestHeader("Authorization") String auth , @PathVariable Integer id) throws CustomException{

        String token = auth.substring(7);
        String username = jwtUtil.extractUsername(token);

        String address = addressService.deleteAddress(username,id);
    ...
    @GetMapping("/address")
    public ResponseEntity<List<Address>> findAllAddress(@RequestHeader("Authorization") String auth) throws CustomException{

        String token = auth.substring(7);

        String username = jwtUtil.extractUsername(token);

        List<Address> addresses = addressService.findAllAddress(username);
